...
public class ChangeRecordTest {
	...
	public void testGetAdditions() {
		for (int i = 0; i < 20; i++) {
			int op = rh.getReleaseByID(i + 1).getChangesRecord(ReleaseInfo.operations).getAdditions();
			assertEquals("Wrong number of operations additions in release " + (i + 1), opAdd[i], op);
			
			int ds = rh.getReleaseByID(i + 1).getChangesRecord(ReleaseInfo.dataStructures).getAdditions();
	...
	public void testGetDeletions() {
		for (int i = 0; i < 20; i++) {
			int op = rh.getReleaseByID(i + 1).getChangesRecord(ReleaseInfo.operations).getDeletions();
			assertEquals("Wrong number of operations deletion in release " + (i + 1), opDel[i], op);
			
			int ds = rh.getReleaseByID(i + 1).getChangesRecord(ReleaseInfo.dataStructures).getDeletions();
	...
	public void testGetChanges() {
		for (int i = 0; i < 20; i++) {
			int op = rh.getReleaseByID(i + 1).getChangesRecord(ReleaseInfo.operations).getChanges();
			assertEquals("Wrong number of operations changes in release " + (i + 1), opChg[i], op);
			
			int ds = rh.getReleaseByID(i + 1).getChangesRecord(ReleaseInfo.dataStructures).getChanges();
