...
public class MetricsRecordTest {
	...
	public void testTotalNumber() {
		...
		for (int i = 0; i < 20; i++) {
			...
			int op = rh.getReleaseInformation(i + 1).getMetricsRecord(ReleaseInformation.operations).getTotalNumber(); // numbering from 1
			assertEquals("Wrong number of total operations in release " + (i + 1), inOp, op);
			
			inDS += dsAdd[i] - dsDel[i];
			int ds = rh.getReleaseInformation(i + 1).getMetricsRecord(ReleaseInformation.dataStructures).getTotalNumber(); // numbering from 1
